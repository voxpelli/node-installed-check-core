...
const checkEngineVersions = function (engines, requiredDependencies, installedDependencies) {
  ...
  Object.keys(requiredDependencies).forEach(dependency => {
    const dependencyEngines = (installedDependencies[dependency] ? installedDependencies[dependency].engines : undefined) || {};
    ...
    engineKeys.forEach(engine => {
      if (!dependencyEngines[engine]) {
        ...
        if (!intersection) {
          ...
          if (value) {
            finalIntersections[engine] = semverIntersect(value, dependencyEngines[engine]);
          }
        }
      }
    });
